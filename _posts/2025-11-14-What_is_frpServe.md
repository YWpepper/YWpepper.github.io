...
> 补充一下： 一台 FRP 服务器 + 两个客户端（完美方案），我们保持[common]不要变。

> 核心规则其他如下

- 一台服务器足够，不用多开
- 两个客户端的 代理名字不能重复 `[pc1_ssh]` `[pc2_ssh]`
- 两个客户端的 端口不能重复 `remote_port = 10001  # 端口1` `remote_port = 10002  # 不同端口`,注意在服务器上面打开那两个端口,命令如下`ufw allow 10001/tcp` `ufw allow 10002/tcp`
- 服务端完全不用改！直接通用。

> 当客户端是wsl里面的ssh

- 注意`wsl`的`ssh`服务已经打开，并且检查`wsl`的`ssh`服务端口，一般不是`22`，可能被我们修改为了`8022`也有可能，所以要注意检查。
- 检查安装指令
  - `apt install openssh-server -y` 安装ssh
  - `service ssh start` 开启服务
  - `systemctl status ssh` 查看状态
- 如果还要开启`rdp`的服务，一般`wsl`是没有这个服务的，所以在`local_ip` 处修改为 `host.docker.internal`，这个需要尝试。
